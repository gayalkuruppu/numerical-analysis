...
print("========== Finding w0, w1, w2 and w3 using RK4 Method ==========")
...
while i<lower_bound + p*step_size_h:
    ...
    i_3 = float("{:.10f}".format(i-(3*step_size_h)))
    wi_1 = float("{:.10f}".format(array[-1] + (step_size_h*((55*Dy(i,array[-1]))
                                                       -(59*Dy(i_1,array[-2]))
                                                            + (37*Dy(i_2,array[-3]))
                                                            -(9*Dy(i_3, array[-4])))/24)))

    print("\nw_" + str(count+1) + "p = " +
          str(array[-1]) + " + " + str(step_size_h) + "[55f(" +
          str(i) + ", " + str(array[-1]) + ") - 59f(" +
          str(i_1) + ", " + str(array[-2]) + ") +37f(" +
          str(i_2) + ", " + str(array[-3]) + ") -9f(" +
          str(i_3) + ", " + str(array[-4]) + ")]/24")
    ...
    print("\nApplying Adams-Moulton 3-step Implicit method as a corrector\n")
    ip1 = float("{:.10f}".format(i+(step_size_h)))
    ans = float("{:.10f}".format(array[-2] + (step_size_h*((9*Dy(ip1, array[-1]))+(19*Dy(
        i, array[-2]))-(5*Dy(i_1, array[-3]))+(Dy(i_2, array[-4])))/24)))

    print("\nw_" + str(count+1) + " = " +
          str(array[-2]) + " + " + str(step_size_h) + "[9f(" +
          str(ip1) + ", " + str(array[-1]) + ") + 19f(" +
          str(i) + ", " + str(array[-2]) + ") - 5f(" +
          str(i_1) + ", " + str(array[-3]) + ") + f(" +
          str(i_2) + ", " + str(array[-4]) + ")]/24")
    print("w_" + str(count+1) + " = " +
          str(array[-2]) + " + " + str(step_size_h) + "(" +
          str(9*Dy(ip1,array[-1])) + " + " + str(19*Dy(i,array[-2])) +
          " - " + str(5*Dy(i_1,array[-3])) + " + " +
          str(Dy(i_2,array[-4])) + ")/24 = " + str(ans))
